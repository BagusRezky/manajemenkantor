...
class ImrDiemaking extends Model
{
    use HasFactory;
    protected $fillable = [
    ...
    {
        return $this->hasMany(ImrDiemakingItem::class, 'id_imr_diemaking');
    }
    ...
    public static function generateNoImr()
    {
        try {
            ...
            $lastImr = self::where('no_imr_diemaking', 'LIKE', 'IMR-' . $year . '%.%')
                ->orderBy('no_imr_diemaking', 'desc')
                ->first();

            if ($lastImr) {
                // Ambil 4 digit terakhir setelah titik dan tambah 1
                $lastPart = substr($lastImr->no_imr_diemaking, strpos($lastImr->no_imr_diemaking, '.') + 1);
