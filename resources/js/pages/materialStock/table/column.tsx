...
export const columns = (): ColumnDef<MaterialStock>[] => [
        ...
        cell: ({ row }) => {
            ...
            return (
                <div className={isLowStock ? 'text-red-600' : ''}>
                    ...
                    <div className={`text-sm ${isLowStock ? 'text-red-600/70' : 'text-gray-500'}`}>{data.nama_type_barang}</div>
                </div>
            );
        },
    },
    ...
    },
    {
        ...
        cell: ({ row }) => {
            const data = row.original; // <- Ambil data original
            const isLowStock = data.onhand_stock < data.min_stock; // <- Logika
            return <span className={`text-right font-mono ${isLowStock ? 'text-red-600' : ''}`}>{row.getValue('min_stock')}</span>;
        },
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return <span className={`text-right font-mono font-bold ${isLowStock ? 'text-red-600' : ''}`}>{value.toFixed(2)}</span>;
        },
    },
    {
