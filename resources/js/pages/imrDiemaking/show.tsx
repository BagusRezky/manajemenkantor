...
export default function ShowInternalMaterialRequest({ imrDiemaking }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Detail IMR - ${imrDiemaking.no_imr_diemaking}`} />
                        ...
                        <CardHeader>
                            <CardTitle className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-3">
                                    <Badge className={getStatusColor(imrDiemaking.status)}>{imrDiemaking.status.toUpperCase()}</Badge>
                        ...
                        <CardContent>
                            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                                <div>
                                    ...
                                    <p className="mt-1 font-mono text-sm text-gray-900">{imrDiemaking.no_imr_diemaking}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p className="mt-1 text-sm text-gray-900">{imrDiemaking.kartu_instruksi_kerja?.production_plan || '-'}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-gray-500">Estimasi Selesai</label>
                                    <p className="mt-1 text-sm text-gray-900">
                                        {imrDiemaking.kartu_instruksi_kerja?.tgl_estimasi_selesai
                                            ? new Date(imrDiemaking.kartu_instruksi_kerja.tgl_estimasi_selesai).toLocaleDateString('id-ID')
                                            : '-'}
                                    </p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-gray-500">Dibuat Pada</label>
                                    <p className="mt-1 text-sm text-gray-900">{formatDate(imrDiemaking.created_at)}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-gray-500">Terakhir Update</label>
                                    <p className="mt-1 text-sm text-gray-900">{formatDate(imrDiemaking.updated_at)}</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
