    ...
    return (
        <div>
            <div className="flex items-center justify-between py-4">
                <Input
                    placeholder="Cari No. Laporan Barang..."
                    value={(table.getColumn('no_laporan_barang')?.getFilterValue() as string) ?? ''}
                    onChange={(event) => table.getColumn('no_laporan_barang')?.setFilterValue(event.target.value)}
                    className="max-w-sm"
                />
                <Link href={route('penerimaanBarangs.create')}>
                    <Button>Tambah Penerimaan Barang</Button>
                </Link>
            </div>
