...
export function DataTable<TData, TValue>({ columns, data }: DataTableProps<TData, TValue>) {
    ...
    const handleImportSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        router.post(route('transFakturs.import'), formData, {
            onSuccess: () => {
        ...
        <div className="space-y-4">
            <div className="flex items-center">
                <Input
                    ...
                    value={(table.getColumn('no_faktur')?.getFilterValue() as string) ?? ''}
                    onChange={(event) => table.getColumn('no_faktur')?.setFilterValue(event.target.value)}
                    className="max-w-sm"
                />
                <div className="ml-auto flex space-x-3">
                    ...
                    <Link href={route('transFakturs.create')}>
                        <Button>
