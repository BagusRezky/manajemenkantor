...
export function DataTable<TData, TValue>({ columns, data }: DataTableProps<TData, TValue>) {
    ...
    const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        router.post(route('erorProductions.import'), formData, {
            onSuccess: () => {
                e.target.value = '';
    ...
    return (
        <div>
            <div className="flex justify-between py-4">
                <Input
                    placeholder="Cari Kode Eror Production..."
                    value={(table.getColumn('kode_eror')?.getFilterValue() as string) ?? ''}
                    onChange={(event) => table.getColumn('kode_eror')?.setFilterValue(event.target.value)}
                    className="max-w-sm"
                />
